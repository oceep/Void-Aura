...
`;
  ...
  while (attemptCount < maxAttempts) {
      try {
          ...
          if (modelMode === 'deep' || (userEnabledSearch && botId !== 'bot-todo-special' && botId !== 'bot-teacher-pro')) {
              tools = [{ googleSearch: {} }];
          }
          ...
          for await (const chunk of responseStream) {
              ...
              if (candidate?.content?.parts) {
                  for (const part of candidate.content.parts) {
                      ...
                      // If the model returns thought via metadata (part.thought)
                      if (p.thought) {
                          if (!isThinking) {
                              yield '<think>';
                              isThinking = true;
                          ...
                          // Close thought block if we were thinking and now switched to text
                          if (isThinking) {
                              yield '</think>';
                              isThinking = false;
                          }
