    ...
    return (
        <div className="mb-4 w-full">
            <div 
                ...
                className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all duration-200 group select-none cursor-pointer ${
                    theme === 'light' 
                    ? 'bg-white border-blue-100 hover:bg-blue-50/50 shadow-sm' 
                    : 'bg-[#1a1a1f] border-gray-800 hover:bg-[#25252b]'
                }`}
            >
                <div className="flex items-center gap-3">
                    <span className={`font-semibold text-base ${theme === 'light' ? 'text-blue-600' : 'text-blue-400'}`}>
                        {t('showThinking')}
                    </span>
                    ...
                    {isStreaming && !duration && (
                        <div className="flex gap-1 ml-1">
                            ...
                            <span className="w-1.5 h-1.5 bg-sky-400 rounded-full animate-bounce" style={{ animationDelay: '0.1s' }}></span>
                ...
                <div className="flex items-center gap-3">
                    ...
                    {isStreaming && !duration && onInstantAnswer && (
                        <button 
                            ...
                            className="text-xs font-bold bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-full transition-all active:scale-95 flex items-center gap-1 shadow-md z-10"
                            title="Trả lời ngay"
                        >
                    ...
                    <svg 
                        className={`w-5 h-5 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''} ${theme === 'light' ? 'text-blue-400' : 'text-gray-400'}`} 
            ...
            {isOpen && (
                 <div className={`mt-2 pl-4 py-2 border-l-4 border-sky-400 bg-transparent ${theme === 'light' ? 'text-gray-700' : 'text-gray-300'} text-base leading-relaxed animate-fade-in`}>
                    {(!content && isStreaming) ? (
                        "..."
                    ) : (
                        <ReactMarkdown 
      ...
      <div className={`w-full mb-6 flex gap-4 group ${isUser ? 'justify-end' : 'justify-start'}`}>
        ...
        >
          ...
          <div className={`markdown-body [&>*:last-child]:mb-0 ${textColorClass} ${showCursor ? 'cursor-blink' : ''}`}>
              {finalVisibleText === '' && message.isStreaming && !thinkingContent && !hasSpecialCard ? (
